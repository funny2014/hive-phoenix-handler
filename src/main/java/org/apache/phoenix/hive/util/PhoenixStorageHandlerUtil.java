	...
	public static Object[] toTypedValues(JobConf jobConf, String typeName, String[] values) throws Exception {
		...
		for (int i = 0, limit = values.length; i < limit; i++) {
			if (serdeConstants.STRING_TYPE_NAME.equals(typeName)) {
				...
				String dateFormat = jobConf.get(PhoenixStorageHandlerConstants.HBASE_DATE_FORMAT, PhoenixStorageHandlerConstants.DEFAULT_DATE_FORMAT);
...
//		Preconditions.checkArgument(columnNames.length == columnTypes.length, "length of column name(" + Lists.newArrayList(columnNames) + ") & column type(" + Lists.newArrayList(columnTypes) + ") is different");
