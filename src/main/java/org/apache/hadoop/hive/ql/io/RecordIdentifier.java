...
public class RecordIdentifier implements WritableComparable<RecordIdentifier> {
  ...
  public static enum Field {
    ...
    primaryKey(TypeInfoFactory.binaryTypeInfo, PrimitiveObjectInspectorFactory.javaByteArrayObjectInspector);
//    primaryKey(TypeInfoFactory.getMapTypeInfo(TypeInfoFactory.stringTypeInfo, TypeInfoFactory.stringTypeInfo), 		// Map<String, String> 으로 해도 됨. 값을 세팅하는 쪽(ex:PhoenixResultWritable>)에서 Map<String, String>으로 넣어줘야 함.
//    		ObjectInspectorFactory.getStandardMapObjectInspector(PrimitiveObjectInspectorFactory.javaStringObjectInspector, PrimitiveObjectInspectorFactory.javaStringObjectInspector));
  ...
  public static final class StructInfo {
    ...
    public static void toArray(RecordIdentifier ri, Object[] struct) {
      ...
      struct[Field.primaryKey.ordinal()] = ri.getPrimaryKeyBytes();
//      struct[Field.primaryKey.ordinal()] = ri.getPrimaryKeyMap();
    }
  }
...
//  }
  ...
  public byte[] getPrimaryKeyBytes() {
	  ...
	  if (primaryKeyMap == null) {
		  primaryKeyBytes = new byte[0];
	  } else {
		  ByteArrayOutputStream baos = new ByteArrayOutputStream();
		  try (ObjectOutputStream oos = new ObjectOutputStream(baos)) {
			  oos.writeObject(primaryKeyMap);
			  primaryKeyBytes = baos.toByteArray();
		  } catch (IOException e) {
